...
def my_forward(self, images, targets=None):
    ...
    #print([torch.any(torch.isnan(features[key])) for key in features.keys()])
    #print(features.keys())
    ...
    #print([torch.any(torch.isnan(proposal_loss)) for proposal_loss in proposal_losses.values()])
    detections, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets)
    detections = self.transform.postprocess(detections, images.image_sizes, original_image_sizes)
    #print([torch.any(torch.isnan (detector_loss)) for detector_loss in detector_losses.values()])
