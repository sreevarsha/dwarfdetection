...
from quicktorch.modules.attention.models import AttentionMS, MSAttMaskGenerator
from quicktorch.modules.attention.loss import GuidedAuxLoss, FocalWithLogitsLoss
...
class ContaminantSegmenter(nn.Module):
    def __init__(self, in_channels, contaminant_idx, base_channels=64, scales=[0, 1, 2, 3, 4], score_thresh=0.2525):
        super().__init__()
        self.contaminant_idx = contaminant_idx
...
class ContaminantAloneSegmenter(ContaminantSegmenter):
    def __init__(self, in_channels, contaminant_idx, base_channels=64, scales=[0, 1, 2, 3, 4]):
        super().__init__(in_channels, contaminant_idx, base_channels, scales)
